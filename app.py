...
with tab0:
    ...
    # ──────────────────────────────────────────────────────────
    # 📅 近 10 天推薦追蹤（只短波段顯示）
    # 仿 Tab 5 投信突襲 expander 風格，點開看每日逐日漲跌 + 累計浮動%
    # 資料來源：recommendation_history.json（Tab 1 每天第一名）
    # ──────────────────────────────────────────────────────────
    if _IS_SHORT:
        st.markdown("---")
        ...
        if not rec_data:
            st.info("尚無推薦追蹤資料（首次跑 pipeline 後出現）")
        else:
            rec_records = rec_data.get("records", [])
            if not rec_records:
                st.info("尚無推薦紀錄")
            else:
                # 摘要表
                ...
                st.caption(f"資料更新：{rec_data.get('updated', '—')}")
...
with tab6:
    if _tab6_active:
        ...
        st.caption("股池為每日成交額前 200 大個股。9 條過濾：綠棒面積 ≥50 + DIF 雙路徑（位置≤0.3 AND DIF/Close≤1.0% OR 5天斜率<0 AND 位置≤0.5）+ OSC 連續縮短 2 天 + 投信 60 天淨買 > 0。追蹤式：同股取首次 sig_date 持續顯示，滿 30 天或 OSC 轉紅自動消失。**排序：綠棒至今天數多→少，同天數時 DIF+DEM 位階低→高**。")

        st.info(
            "🕕 **每天 18:00 自動更新**（含 19:00 / 20:00 兩道 retry）。\n"
            "捕捉投信加碼 + MACD 大綠棒收尾 + 位階低的早期反轉訊號。"
        )
        ...
        if not macd_trust_data:
            ...
            st.markdown(
                ...
                "滿 30 天或 OSC 轉紅自動消失。**排序：綠棒至今天數多 → 少（同天數時位階低 → 高）**。"
            )
