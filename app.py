...
with tab6:
    if _tab6_active:
        ...
        st.caption("股池為每日成交額前 200 大個股。9 條過濾：綠棒面積 ≥50 + DIF 雙路徑（位置≤0.3 AND DIF/Close≤1.0% OR 5天斜率<0 AND 位置≤0.5）+ OSC 連續縮短 2 天 + 投信 60 天淨買 > 0。追蹤式：同股取首次 sig_date 持續顯示，滿 30 天或 OSC 轉紅自動消失。排序按當下 DIF+DEM 半年位階升冪（位階低 → 在前）。")
